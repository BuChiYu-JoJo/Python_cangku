...
URLS = ["http://ipinfo.io/"]
#URLS = ["https://ipinfo.io/", "http://myip.lunaproxy.io/"]
...
async def fetch(target_url, index):
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        try:
            ...
            async with session.post(API_ENDPOINT, headers=HEADERS, data=payload, ssl=False) as response:
                ...
                print(f"Completed {index} - {target_url} ({elapsed:.2f}s, {size_kb} KB)")
...
def save_to_csv():
    with open(OUTPUT_CSV, mode="w", newline='', encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["url", "response_time", "file_size"])
        writer.writeheader()
