...
BATCH_SIZE = 10000             # 写 CSV 的批大小（缓存到这个数量后写一次）
CSV_FILE = "mei_na.csv"    # 明细输出 CSV 文件名
SUMMARY_CSV_FILE = "summary_mei_na.csv"  # 汇总输出 CSV 文件名

TARGET_URL = "http://ipinfo.io/json"   # 目标 URL（固定）
REQUEST_TIMEOUT = 15        # 单次请求超时（秒）

PROXY: Optional[str] = "http://td-customer-MzsNH4f:EtXApbeko8bDT@rmmsg2sa.na.thordata.net:9999"
# 若不需代理设为 None
# -----------------------
...
#    connector = TCPConnector(limit=0, ssl=False)  # 并发由 semaphore 控制
    ...
    limit=0,
    ssl=False,
    force_close=True,        # 强制每次请求后关闭连接（不复用）
