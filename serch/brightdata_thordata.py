...
# ✨ 新增：服务选择配置（可选值："thordata", "brightdata"）
SERVICES_TO_RUN = [
#    "thordata",    # Thordata 服务（Bing搜索）
    "brightdata"   # Brightdata 服务（Google搜索）
]
 ...
 # 服务定制化校验逻辑
        if response.status_code == 200:
            if len(content) >= MIN_CONTENT_LENGTH:
                success = True  # 明确设置成功条件
                error_info = ""  # 清空错误信息
            else:
                success = False  # 明确设置失败条件
                error_info = f"Content below {MIN_CONTENT_LENGTH} bytes"
        else:
            success = False  # 明确设置失败条件
        ...
        if success:
            ...
            recorder.log_request(req_id, query, True, latency, len(content), error_info)
            return True
        else:
            monitor.record_failure(latency)
            recorder.log_request(req_id, query, False, latency, len(content), error_info)
            save_error(service_name, query, req_id, response.status_code, content)
            return False

    except Exception as e:
        latency = (time.time() - start_time) * 1000
        error_info = f"{type(e).__name__}: {str(e)}"  # ✨ 异常信息格式化
        monitor.record_failure(latency)
        recorder.log_request(req_id, query, False, latency, 0, error_info)
